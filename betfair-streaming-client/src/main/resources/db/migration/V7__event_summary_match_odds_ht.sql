-- Replace HALF_TIME_RESULT with MATCH_ODDS_HT in v_event_summary (m3 = half-time market slot).
-- Column names half_time_market_id / half_time_market_name retained for compatibility.
...
LEFT JOIN (
    SELECT DISTINCT ON (event_id) event_id, market_id, market_name
    FROM markets WHERE market_type = 'MATCH_ODDS_HT'
    ORDER BY event_id, market_start_time ASC NULLS LAST, market_id ASC
